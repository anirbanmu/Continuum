...
void Maze::subdivide_grid(Maze& maze, mt19937& mersenne_twister, tuple<unsigned, unsigned> rect_start, tuple<unsigned, unsigned> rect_end)
{
    ...
    if (horizontal)
    {
        const unsigned division_y = get<1>(rect_start) + max(min(mersenne_twister() % get<1>(rect_size), static_cast<unsigned long>(get<1>(rect_size) - 4)), static_cast<unsigned long>(4));
        for (unsigned x = get<0>(rect_start); x < get<0>(rect_end); ++x)
        {
            maze.cell(x, division_y) = &maze.wall;
        }
    ...
    const unsigned division_x = get<0>(rect_start) + max(min(mersenne_twister() % get<0>(rect_size), static_cast<unsigned long>(get<0>(rect_size) - 4)), static_cast<unsigned long>(4));
    for (unsigned y = get<1>(rect_start); y < get<1>(rect_end); ++y)
    {
        maze.cell(division_x, y) = &maze.wall;
    }
